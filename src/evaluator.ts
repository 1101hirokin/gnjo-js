import AST from "./ast";
import { ColorSpace, HSLASpace, HWBSpace, LabSpace, RGBASpace, ValueWithAngleUnit, XYZSpace } from "./colorSpace";

export class Evaluator {
  public readonly color: AST.Color
  constructor(color: AST.Color) {
    this.color = color
  }

  public evaluate(): ColorSpace {
  ...
  private _evalValueNode(node: AST.ValueNode): ValueWithAngleUnit {
    switch (node.unit) {
      case "deg":
  ...
  private _evalNamedColorNode(node: AST.Node): RGBASpace {
    const namedColor = node as AST.NamedColorNode
  ...
  private _evalHexNode(node: AST.Node): RGBASpace {
    const hex = node as AST.HEXNode
    return new RGBASpace(
      parseInt(hex.r, 16),
      parseInt(hex.g, 16),
      parseInt(hex.b, 16),
      hex.alpha ? parseFloat(hex.alpha) : undefined
    )
  }
  private _evalRGBANode(node: AST.Node): RGBASpace {
    const rgb = node as AST.RGBANode
    if (rgb.r.unit !== "") { throw new Error("Invalid unit for red value") }
    if (rgb.g.unit !== "") { throw new Error("Invalid unit for green value") }
    if (rgb.b.unit !== "") { throw new Error("Invalid unit for blue value") }

    return new RGBASpace(
      this._evalValueNode(rgb.r).value,
      this._evalValueNode(rgb.g).value,
      this._evalValueNode(rgb.b).value,
      rgb.alpha ? this._evalValueNode(rgb.alpha).value : undefined
    )
  }
  private _evalHSLANode(node: AST.Node): HSLASpace {
    const hsla = node as AST.HSLANode
    return new HSLASpace(
      this._evalValueNode(hsla.h),
      this._evalValueNode(hsla.s).value,
      this._evalValueNode(hsla.l).value,
      hsla.alpha ? this._evalValueNode(hsla.alpha).value : undefined
    )
  }
  private _evalHWBNode(node: AST.Node): HWBSpace {
    const hwb = node as AST.HWBNode
    return new HWBSpace(
      this._evalValueNode(hwb.h),
      this._evalValueNode(hwb.w).value,
      this._evalValueNode(hwb.b).value,
      hwb.alpha ? this._evalValueNode(hwb.alpha).value : undefined
    )
  }
  private _evalXYZNode(node: AST.Node): XYZSpace {
    const xyz = node as AST.XYZNode
    return new XYZSpace(
      this._evalValueNode(xyz.x).value,
      this._evalValueNode(xyz.y).value,
      this._evalValueNode(xyz.z).value,
      xyz.alpha ? this._evalValueNode(xyz.alpha).value : undefined
    )
  }

  public _evalLabNode(node: AST.Node): LabSpace {
    const lab = node as AST.LabNode
    return new LabSpace(
      this._evalValueNode(lab.l).value,
      this._evalValueNode(lab.a).value,
      this._evalValueNode(lab.b).value,
      lab.alpha ? this._evalValueNode(lab.alpha).value : undefined
    )
  }
}
